...
#        axes[1].plot(delays[::30], RMSD_X[::30], color = ColorStyle[Organism], marker = MarkerStyle[Organism], MarkerSize = 10, label = Organism + Condition, alpha = 0.7)
        ...
        axes[0].plot(delays[1::30],data['MSD_Z'][1::30], color = ColorStyle[Organism], marker = MarkerStyle[Organism], MarkerSize = 10, label = Organism + Condition, alpha = 0.7)
        axes[1].plot(delays[1::30], data['MSD_X'][1::30], color = ColorStyle[Organism], marker = MarkerStyle[Organism], MarkerSize = 10, label = Organism + Condition, alpha = 0.7)
#        
        # Plots of Lines
#         axes[0].plot(delays,MSD_Z, color = ColorStyle[Organism], linewidth = 2, label = Organism, alpha = 0.7)
#         axes[1].plot(delays, MSD_X, color = ColorStyle[Organism], linewidth = 2, label = Organism, alpha = 0.7)
...
a=1
b=1
axes[0].plot(a*delays[1:100],delays[1:100]**2,'k-')
axes[0].plot(b*delays[100:],delays[100:],'k--') 

axes[1].plot(a*delays[1:100],delays[1:100]**2,'k-')
axes[1].plot(b*delays[100:],delays[100:],'k--') 

axes[0].set_xlabel('Time (s)')
axes[1].set_xlabel('Time (s)')

axes[0].set_ylabel('MSD (mm^2)')
...
plt.savefig(os.path.join(msd_folder, 'Combined_MSD_log-log.svg'), dpi = 150)
plt.savefig(os.path.join(msd_folder,'Combined_MSD_log-log.png'), dpi = 150)

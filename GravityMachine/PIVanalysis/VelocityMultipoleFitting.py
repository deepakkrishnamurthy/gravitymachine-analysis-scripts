...
def readVelocityFitData(filename):
    with(open(filename,'rb')) as f:
        p_x, p_y, r_c_x, r_c_y, U_0, A_st, A_str, A_sd, relError =  pickle.load(f)
    
    return p_x, p_y, r_c_x, r_c_y, U_0, A_st, A_str, A_sd, relError
...
#                p_y = value
    ...
    #--------------------------------------------------------------------------
    p_x, p_y, r_c_x, r_c_y, U , A_st, A_str, A_sd = fit_params
...
def testVelocityField(objDia=0.2,r_centroid = [0,0], p=[0,1],U_trans = 0,A_st = 1e-2,A_str = 0,A_sd = 0):
    #==============================================================================
...
def main():
    ...
    if testFlag == 1:
        ...
        A_str = 1
        A_sd = 1
        fit_params_0 = [p_x, p_y, r_c_x, r_c_y, U_0, A_st, A_str, A_sd] 
       
        fit_params_min = [-1, -1, r_c_x - objDia/2,  r_c_y - objDia/2, -np.inf, -np.inf, -np.inf, -np.inf ]
        fit_params_max = [1, 1,  r_c_x + objDia/2, r_c_y + objDia/2, np.inf, np.inf, np.inf, np.inf ]
                      
        
        res_1 = least_squares(computeVelocityError, fit_params_0, bounds = (fit_params_min, fit_params_max), kwargs = {'X_exp':x_test, 'Y_exp':y_test, 'U_exp':u_test, 'V_exp':v_test}, max_nfev = 100)
        p_x_fit, p_y_fit, r_x_fit, r_y_fit, U_trans_fit, A_st_fit, A_str_fit, A_sd_fit = res_1.x 
        ...
        plotVelocityField(x_test, y_test, u_fit, v_fit)
     
        
        plotScalarField(x_test, y_test, 100*velErrorMag/U_exp_mag)
...
#        Exp_dataPath = '/Volumes/GRAVMACH1 1/HopkinsEmbroyologyCourse_GoodData/2018_06_07/seacucmber4_auto_verylong_goodtrack'
        ...
        # Starfish
        #--------------------------------------------------------------------------
#        Exp_dataPath = '/Volumes/GRAVMACH1 2/HopkinsEmbroyologyCourse_GoodData/2018_06_12/Starfish/StarFish6highfreq'
    ...
    #    data_path = '/Volumes/GRAVMACH1/HopkinsEmbroyologyCourse_GoodData/2018_06_13/Snail/snail2'
        ...
        #--------------------------------------------------------------------------
        # Dendraster
        Exp_dataPath = '/Volumes/GRAVMACH1 2/HopkinsEmbroyologyCourse_GoodData/2018_06_11/Dendraster_starved_11Days_nofood/Dendraster3'

        
        imageFolder = 'images'
        
        rootFolder = '/Volumes/GRAVMACH1 2/GravityMachine/Results/PIV_Results'
...
#        dataFolder = 'Dendraster3_IMG_6480_6502'
        ...
        #--------------------------------------------------------------------------
        resultFolder = os.path.join(rootFolder,dataFolder,'VelocityFitOld')
...
#            plotPIVdata(frame_color, x_fine,y_fine, u_fine, v_fine, U_fine, orgContour, figname = 1)
            ...
            # velocity fit file
            #--------------------------------------------------------------------------
            saveFile = os.path.join(resultFolder,'velocityFit' + img_name+'.pkl')
            #--------------------------------------------------------------------------
    ...
    #    
            ...
            #    --------------------------------------------------------------------------
                ...
                if(ii>0):
                    ...
                    fit_params_0 = [p_x_fit, p_y_fit, r_x_fit, r_y_fit, U_trans_fit, A_st_fit, A_str_fit, A_sd_fit]
                else:
                    fit_params_0 = [px_0, py_0, rx_0, ry_0, U_0, A_st_0, A_str_0, A_sd_0]
                ...
                fit_params_min = [0, 0, rx_0 - objDia/2,  ry_0 - objDia/2, -np.inf, -np.inf, -np.inf, -np.inf ]
                fit_params_max = [1, 1,  rx_0 + objDia/2, ry_0 + objDia/2, np.inf, np.inf, np.inf, np.inf ]
...
#                fit_params_max = [1, 1,  np.inf, np.inf , np.inf, np.inf, np.inf, np.inf ]
                ...
                with open(saveFile, 'wb') as f:  # Python 3: open(..., 'wb')
                            pickle.dump((p_x_fit, p_y_fit, r_x_fit, r_y_fit, U_trans_fit, A_st_fit, A_str_fit, A_sd_fit, relError), f)
    #            
                #--------------------------------------------------------------------------
                # If the data already exists then read it into memory
                #--------------------------------------------------------------------------
            else:
                p_x_fit, p_y_fit, r_x_fit, r_y_fit, U_trans_fit, A_st_fit, A_str_fit, A_sd_fit, relError = readVelocityFitData(saveFile)
    ...
    #            
            ...
            # Compute the "Fitted" velocity field
            u_fit, v_fit = computeFittedVelocity(x_real, y_real, p =[p_x_fit, p_y_fit],r_c = [r_x_fit, r_y_fit], U = U_trans_fit, A_st = A_st_fit, A_str = A_str_fit, A_sd = A_sd_fit)
            # Find norm of residual vector
...
#                plt.arrow(pixelPermm(imW)*rx_0,pixelPermm(imW)*(imH*mmPerPixel(imW)-ry_0), 50*p_x_fit, 50*-p_y_fit, head_width=5, head_length=5, fc='r', ec='r')    
...
#            u_lab, v_lab = (u + p_x_fit*U_trans_fit, v + p_y_fit*U_trans_fit )
...
#            plt.imshow(frame_color)
#                
#            ax1 = plt.contourf(x,y,U_lab)
#            plt.quiver(x,y,u_lab,v_lab)
#            cbar = plt.colorbar(ax1)
#            cbar.ax.set_ylabel('Velocity magnitude (mm/s)')
...
#        plt.legend(handles=[ax1, ax2])
        ...
        plt.figure(5)
        plt.subplot(221)
        ax1 = plt.scatter(indexArray[errorFilter],U_array[errorFilter],20,color='k')
        plt.xlabel('Image Index')
        ...
        U_trans_mean, U_trans_std = (np.nanmean(U_array[errorFilter]), np.nanstd(U_array[errorFilter]))
        ...
        print('Translational velocity (Mean +- Std) {} +- {} mm/s'.format(np.nanmean(U_array[errorFilter]), np.nanstd(U_array[errorFilter] )))
    ...
    #                
        ...
        with open(os.path.join(rootFolder, dataFolder,fileName_mean_std), 'wb') as f:  # Python 3: open(..., 'wb')
            pickle.dump((objDia_mean, objDia_std, U_trans_mean, U_trans_std,A_st_mean, A_st_std,errorBound) , f)
    #            
